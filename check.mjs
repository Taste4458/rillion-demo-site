...
assert.doesNotMatch(app.match(/\{ group: 'Business Reports', title: 'AP cash flow'[^\n]+/)[0], /leftRail/, 'AP cash flow must preserve its full native sidebar');
...
assert.match(css, /\.analytics-image-frame--left-rail::before\{[^}]*width:7\.5%;[^}]*background:var\(--wash\)/, 'Dark-edge Analytics captures need the Spend Report gray left rail');
