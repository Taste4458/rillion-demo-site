...
Payments follows the Demo51 operational sequence with four native tab controls: Ready for payment, Awaiting approval, In progress, and Completed. Dynamic tab counts and totals reflect the current synthetic browser state. A shared filter row narrows records by invoice, vendor, payment method, and date; a payment-specific CSV export respects the active view. Ready for payment gates Pay outside Rillion and Send for payment behind invoice selection, then simulates the selected transition locally. The canonical Precision Tools invoice appears only after manager approval and completes the connected journey when sent. Completed exposes payment dates, reference IDs, settled status, and a reference-review action. Manage and Payment portal remain truthful simulated actions confirmed by toasts. Wide payment tables scroll inside their surface and never widen the 390px page.

### Guided Tour

Every fresh link load opens a branded, dismissible welcome dialog. Accounts payable, Invoice approver, and Finance leader choices open task-specific walkthroughs; the eight-step full-platform path opens one AI-match example and one purchase-order variance after Invoice Log, and five secondary buttons jump directly to To Verify, Documents, Contracts, Payments, or Analytics. “Explore on my own,” the backdrop, and Escape dismiss the welcome. The modal traps focus, makes the background inert and hidden from assistive technology, restores focus on close, and announces progress through a polite live region. The completed walkthrough offers Rillion's official Request a live demo link plus a Continue exploring action. The persistent launch control remains visible in the top bar and opens the walkthrough chooser.

Shareable scenario links encode `scenario`, `view`, and `welcome=0`, landing directly in the relevant module with a visible scenario banner and a control to start that persona's walkthrough. Invoice detail links additionally encode `invoice`; purchase-order detail links encode both `invoice` and `po`, so a reload preserves the evidence record being presented.
...
Invoice Log follows To Verify in both navigation and the guided tour. Its horizontally contained operational grid mirrors Demo51's evidence density and live column order, adding a compact Code column sourced from the same invoice record used by detail. Automated flow/account proposals use labeled green, yellow, or red chips, overdue dates use the real pale-critical treatment, and monetary columns use aligned tabular numerals. PO and match use the same labeled state language: green means fully matched, yellow identifies a Delivery variant, light orange identifies a Price variant, and red means exception or missing PO. Confidence percentages are exclusive to AI matching: above 80% is green, 50–80% inclusive is yellow, and below 50% is red. Purchase-order fully matched, delivery variant, price variant, and exception outcomes show no confidence. Vendor names are native invoice links that open a two-pane invoice workspace with persistent fields and code, the source invoice image, and full-width Invoice lines, Account posting, and Comments/attachments tabs. AI line codes inherit the confidence tone and sit beside coding and flow-proposal rationales. PO references inherit the PO outcome tone; hovering or focusing the reference reveals ordered, delivered, invoiced, and price evidence, while the adjacent semantic info control opens a contained synthetic purchase-order detail. A selection-gated Transfer action moves only eligible records into the matching role's Approval Inbound queue; already transferred, direct-recording, and log-only records cannot be selected. `Directly to recording` remains reserved for fully matched purchase orders and contracts. Legitimate non-PO examples show `AI generated` in both proposal columns and pair `Non-PO verified` with `AI matched`. Text and a dot accompany every color so state never depends on color alone.

The AP and full-platform walkthroughs open `INV-82416` for AI coding evidence and `INV-80116` / purchase order `4500847` for price-variance evidence. These same examples are the runtime smoke-check anchors, keeping the tour, deep links, and test coverage on one shared product path.

### Analytics

Rillion Analytics uses a grouped board index beside one dominant, source-faithful board image. Eleven product views follow the real platform hierarchy: AP Reports (Invoice Log, Active Invoices, AP Aging), Performance Tracking Reports (Invoice Flow Tracking, Invoice Summary), Business Reports (Spend Report, AP Cash Flow, Vendor Payment Analyzer), Executive Dashboard, and Procurement Reports (Procurement Overview, Procurement Trend). Every reference image lazy-loads with fixed dimensions to avoid unnecessary transfer and layout shift; Previous, Next board, and clicking the board itself advance through the real sequence.
