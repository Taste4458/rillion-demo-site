...
Rillion Analytics uses a grouped board index beside one dominant board. Eleven product views follow the real platform hierarchy: AP Reports (Invoice Log, Active Invoices, AP Aging), Performance Tracking Reports (Invoice Flow Tracking, Invoice Summary), Business Reports (Spend Report, AP Cash Flow, Vendor Payment Analyzer), Executive Dashboard, and Procurement Reports (Procurement Overview, Procurement Trend). Active Invoices, Vendor Payment Analyzer, and Executive Dashboard are native interactive synthetic boards with All companies, Company 30, and Last quarter filters plus a reference-view toggle. The remaining boards are source-faithful screenshots, and every reference image lazy-loads with fixed dimensions to avoid unnecessary transfer and layout shift.
