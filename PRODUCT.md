...
- Invoices includes interactive To Verify and Invoice Log branches; Invoice Log follows the real column order through tax, information, purchase order, and contract, exposes labeled flow/accounting proposals, preserves green/yellow/red PO and match states, routes only fully matched purchase orders and contracts directly to recording, and gives log-only non-PO examples AI-generated flow and account-posting proposals; Reports contains AP performance; Rillion Analytics includes eleven board click-throughs grouped in platform order.
