...
function invoiceLog() {
  ...
  const rows = matches.map(inv => `<tr data-invoice="${inv.id}" tabindex="0" role="link" aria-label="Open ${inv.id} from ${inv.vendor}"><td>30</td><td><strong>${inv.vendor}</strong><small>${inv.id}</small></td><td>${inv.vendorInvoice}</td><td>${inv.flowProposal}</td><td>${inv.accountPosting}</td><td>${inv.accountingDate}</td><td>${inv.due}</td><td class="numeric">${money(inv.amount)}</td><td>${inv.currency}</td><td><strong>${inv.po}</strong>${health(inv.poTone, inv.po === 'Non-PO' ? 'Non-PO verified' : inv.poTone === 'good' ? 'PO matched' : inv.poTone === 'warn' ? 'PO variance' : 'PO missing')}</td><td>${health(inv.matchTone, inv.aiMatched ? `AI matched ${inv.match}%` : inv.matchTone === 'good' ? `Matched ${inv.match}%` : inv.matchTone === 'warn' ? `Review ${inv.match}%` : `Exception ${inv.match}%`)}</td><td>${status(inv)}</td></tr>`).join('');
  workspace.innerHTML = pageIntro('Invoice Log', 'Follow every captured invoice from AP review through approval and transfer.', '<button class="primary-button" data-view="to-verify">Open To Verify</button>') + `<div class="invoice-log-toolbar"><div class="filter-tabs" aria-label="Filter invoice log">${filters.map(([id, label]) => `<button data-log-filter="${id}" aria-pressed="${state.logFilter === id}">${label}</button>`).join('')}</div><div class="log-legend" aria-label="Invoice log status key">${health('good', 'Matched')}${health('warn', 'Review')}${health('bad', 'Exception')}</div></div><section class="panel table-wrap invoice-log-table">${rows ? dataTable(['Company', 'Vendor', 'Vendor invoice no.', 'Flow proposal', 'Account posting', 'Accounting date', 'Due date', 'Total', 'Currency', 'Purchase order', 'Match', 'Status'], rows) : emptyState('No matching invoices', 'Clear the search or choose another status.')}</section>`;
}
