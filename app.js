...
const analyticsReports = [
  ...
  { group: 'AP Reports', title: 'Active invoices', image: 'assets/analytics/active-invoices.png', copy: 'See active invoice volume, flow status, invoice type, match status, role bottlenecks, and the underlying invoice list.', capture: true },
  ...
  { group: 'Performance Tracking Reports', title: 'Invoice flow tracking', image: 'assets/analytics/invoice-flow-tracking.png', copy: 'Follow invoice-flow activity, role processing time, users, bottlenecks, and the detailed transfer history.', capture: true },
  ...
  { group: 'Business Reports', title: 'Vendor payment analyzer', image: 'assets/analytics/vendor-payment-analyzer.png', copy: 'Compare vendor payment timing, terms, late and early payments, and invoice volume across companies.', capture: true },
  ...
  { group: 'Procurement Reports', title: 'Procurement overview', image: 'assets/analytics/procurement-overview.png', copy: 'Review purchase-order, delivery, invoice-match, currency, and item performance in one procurement view.', capture: true },
  { group: 'Procurement Reports', title: 'Procurement trend', image: 'assets/analytics/procurement-trend.png', copy: 'Explore adjusted purchasing amounts over time with matching, vendor, order, item, and company filters.', capture: true }
];
...
function invoiceLog() {
  ...
  const rows = matches.map(inv => `<tr data-invoice="${inv.id}" tabindex="0" role="link" aria-label="Open ${inv.id} from ${inv.vendor}"><td>30</td><td><strong>${inv.vendor}</strong><small>${inv.id}</small></td><td>${inv.vendorInvoice}</td><td>${inv.flowProposal}</td><td>${inv.accountPosting}</td><td>${inv.accountingDate}</td><td>${inv.due}</td><td class="numeric">${money(inv.amount)}</td><td>${inv.currency}</td><td><strong>${inv.po}</strong>${health(inv.poTone, inv.poTone === 'good' ? 'PO matched' : inv.poTone === 'warn' ? 'PO variance' : 'PO missing')}</td><td>${health(inv.matchTone, inv.matchTone === 'good' ? `Matched ${inv.match}%` : inv.matchTone === 'warn' ? `Review ${inv.match}%` : `Exception ${inv.match}%`)}</td><td>${status(inv)}</td></tr>`).join('');
...
const tours = [
  ...
  ['Explore Analytics', 'Move through ten boards in the same groups and order as the Rillion Analytics platform.', 'analytics']
];
